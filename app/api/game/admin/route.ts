...
import { jumpToStep, scenariosById, type ScenarioParams } from '@/lib/game/dev/scenarios'
import { appendEvents, listEvents, loadSave, wipeSave } from '@/lib/game/store'
import { resettableSystems, type NewGameEvent, type ResettableSystem, type SaveFile } from '@/lib/game/types'
...
const toEvents = (body: AdminAction, save: SaveFile): NewGameEvent[] | string => {
  switch (body.action) {
    case 'rename':
      if (!body.name?.trim()) return 'name is required'
      return [{ payload: { type: 'player.renamed', name: body.name.trim().slice(0, 40) } }]
    case 'xp':
    ...
    case 'set-trait': {
      ...
      return delta === 0 ? [] : [{ payload: { type: 'trait.changed', trait: body.trait, delta, reason: 'admin' } }]
    }
    case 'achievement':
      if (!achievementsById[body.achievementId]) return `No such achievement: ${body.achievementId}`
      return [{ payload: body.unlocked ? { type: 'achievement.unlocked', achievementId: body.achievementId, reason: 'admin' } : { type: 'achievement.revoked', achievementId: body.achievementId, reason: 'admin' } }]
    ...
    case 'remove-item': {
      const quantity = body.quantity ?? 1
      if (!getItem(body.itemId)) return `No such item: ${body.itemId}`
      if (!Number.isInteger(quantity) || quantity < 1) return 'quantity must be a positive integer'
      return [{ payload: { type: 'item.consumed', itemId: body.itemId, quantity, reason: 'admin' } }]
    }
    case 'rearm':
      return [{ payload: { type: 'grants.rearmed' } }]
    case 'accept-all': {
      const pending = save.player.grants.filter((g) => !g.acceptedAt).map((g) => g.key)
      return pending.length ? [{ payload: { type: 'grants.accepted', grantKeys: pending } }] : []
    }
    ...
    case 'jump-quest': {
      ...
      return jumpToStep(quest, body.step, save.quests[body.slug]?.answers ?? {})
    }
    case 'reset-step': {
      const quest = getQuest(body.slug)
      if (!quest) return `No such quest: ${body.slug}`
      ...
      return [{ payload: { type: 'quest.progressed', slug: body.slug, step: Math.min(current.step, index), answers, unlockedSteps: current.unlockedSteps.filter((id) => id !== body.stepId) } }]
    }
    case 'mark-complete':
      if (!getQuest(body.slug)) return `No such quest: ${body.slug}`
      return [{ payload: { type: 'quest.completed', slug: body.slug, answers: save.quests[body.slug]?.answers ?? {} } }]
    case 'reset-quest':
      if (!getQuest(body.slug)) return `No such quest: ${body.slug}`
    ...
    case 'relock-quest':
      if (!getQuest(body.slug)) return `No such quest: ${body.slug}`
      return [{ payload: { type: 'quest.relocked', slug: body.slug, reason: 'admin' } }]

    case 'flag': {
      const id = body.id?.trim()
      if (!id) return 'id is required'
      if (body.kind === 'location') return [{ payload: body.on ? { type: 'location.discovered', locationId: id, reason: 'admin' } : { type: 'location.forgotten', locationId: id, reason: 'admin' } }]
      return [{ payload: body.on ? { type: 'secret.found', secretId: id, reason: 'admin' } : { type: 'secret.forgotten', secretId: id, reason: 'admin' } }]
    }

    case 'reset-system':
      if (!resettableSystems.includes(body.system)) return `No such system: ${body.system}`
      return [{ payload: { type: 'system.reset', system: body.system, reason: 'admin' } }]
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'wipe') {
      return NextResponse.json({ applied: [], view: buildView(await wipeSave()) })
    }
    if (body.action === 'scenario') {
      const scenario = scenariosById[body.id]
      if (!scenario) return NextResponse.json({ error: `No such scenario: ${body.id}` }, { status: 400 })
      await wipeSave()
      const { save, applied } = await appendEvents(scenario.events(body.params ?? {}))
