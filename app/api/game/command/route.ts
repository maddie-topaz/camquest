...
export async function POST(request: NextRequest) {
  ...
  if (!command || typeof command !== 'object' || !commandTypes.has(command.type)) {
    return NextResponse.json({ error: 'A valid command is required' }, { status: 400 })
  }

  try {
    const outcome = await runCommand(command)
    if (!outcome.ok) {
      return NextResponse.json({ rejection: outcome.rejection, view: buildView(outcome.save) }, { status: 409 })
    }
    return NextResponse.json({ applied: outcome.applied, view: buildView(outcome.save) })
  } catch (error) {
