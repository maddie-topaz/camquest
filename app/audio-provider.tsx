...
export function AudioProvider({ children }: { children: React.ReactNode }) {
  ...
  // Match the server's initial markup; restore the device preference only
  // after hydration. Compare before toggling so Strict Mode is idempotent.
  const actor = useActorRef(audioMachine, { input: { player, muted: false } });
  const muted = useSelector(actor, (snapshot) => snapshot.context.muted);
  const unlocked = useSelector(
    actor,
    (snapshot) => !snapshot.matches("locked"),
  );

  useEffect(() => {
    if (readMuted() !== actor.getSnapshot().context.muted)
      actor.send({ type: "TOGGLE_MUTE" });
  }, [actor]);

  useEffect(() => {
    let cancelled = false;
    void import("@/lib/game/audio/howler-player").then(
      ({ createHowlerPlayer }) => {
        if (cancelled) return;
        real.current = createHowlerPlayer();
        real.current.setVolume(
          actor.getSnapshot().context.muted
            ? 0
            : actor.getSnapshot().context.volume,
        );
      },
    );
    return () => {
  ...
  useEffect(
    () =>
      subscribeCues(({ cue, detail }) =>
        actor.send({ type: "CUE", cue, detail }),
      ),
    [actor],
  );
