...
const useRunner = () => {
  ...
  const note = useCallback(
    (text: string, ok: boolean) =>
      setLog((current) =>
        [{ at: Date.now(), text, ok }, ...current].slice(0, 8),
      ),
    [],
  );
  ...
  const engine = useCallback(
    (label: string, command: Command) => run(label, () => dispatch(command)),
    [dispatch, run],
  );
      ...
      <div className="dev-grid">
        <div className="dev-row">
          ...
          <Btn
            ...
            label="Rename"
            busy={r.busy}
            onClick={() => r.force("Rename", { action: "rename", name })}
          />
        </div>
        <div className="dev-row">
          <span>
            Level {view.level.level} · {view.save.player.xp} XP (
            {view.level.xp - view.level.floor}/
            {view.level.ceiling - view.level.floor} to next)
          </span>
          <label>
            Δ XP{" "}
            <input
              className="dev-input is-short"
              value={xp}
          ...
          <Btn
            ...
            busy={r.busy}
            onClick={() =>
              r.force("Add XP", { action: "xp", amount: Number(xp) })
            }
          />
        </div>
        {traits.map((trait) => {
          ...
          return (
            <div className="dev-row" key={trait.id}>
              <span title={trait.description}>
                {trait.name} <strong>{value}</strong>{" "}
                <small>/{trait.max}</small>
              </span>
              <Btn
                mode="force"
                label={`${trait.name} −1`}
                busy={r.busy}
                onClick={() =>
                  r.force(`${trait.name} −1`, {
                    action: "trait",
                    trait: trait.id,
                    delta: -1,
                  })
                }
                disabled={value <= trait.min}
              />
              <Btn
                mode="force"
                label={`${trait.name} +1`}
                ...
                disabled={value >= trait.max}
              />
              <Btn
                mode="force"
                label={`${trait.name} reset`}
                busy={r.busy}
                onClick={() =>
                  r.force(`${trait.name} reset`, {
                    action: "set-trait",
                    trait: trait.id,
                    value: trait.initial,
                  })
          ...
          {achievements.map((a) => {
            ...
            return (
              <Btn
                ...
                title={a.description}
                onClick={() =>
                  r.force(`${on ? "Revoke" : "Unlock"} ${a.name}`, {
                    action: "achievement",
                    achievementId: a.id,
                    unlocked: !on,
                  })
...
function InventorySection({ view, r }: { view: SaveView; r: Runner }) {
  ...
  return (
    <Section
      id="inventory"
      title="Inventory"
      blurb="Grant and remove are raw; Consume goes through the engine and respects the consumable rule. Accept is the same command the START reveal sends."
    >
      <div className="dev-grid">
        <div className="dev-row">
          <span>
            {view.pendingGrants.length} grant
            {view.pendingGrants.length === 1 ? "" : "s"} pending accept
          </span>
          <Btn
            mode="engine"
            ...
            busy={r.busy}
            onClick={() =>
              r.engine("Accept all pending", {
                type: "grants.accept",
                grantKeys: pendingKeys,
              })
            }
          ...
          <Btn
            ...
            label="Re-arm reveal"
            busy={r.busy}
            onClick={() => r.force("Re-arm reveal", { action: "rearm" })}
          />
        </div>
        {items.map((item) => {
          ...
          return (
            <div className="dev-row" key={item.id}>
              ...
              <Btn
                mode="engine"
                label={`Consume ${item.id}`}
                busy={r.busy}
                onClick={() =>
                  r.engine(`Consume ${item.id}`, {
                    type: "item.consume",
                    itemId: item.id,
                    quantity: 1,
                    reason: "dev",
...
function QuestsSection({ view, r }: { view: SaveView; r: Runner }) {
  ...
  return (
    <Section
      id="quests"
      title="Quests"
      blurb="Start and Complete are the real commands (Complete pays rewards, needs requirements met). Everything else forces the save: jumping to a step unlocks the passcodes before it."
    >
      {quests.map((quest) => {
        ...
        const passcodes = [
          ...(quest.startPasscode
            ? [{ id: "start", label: "Start", code: quest.startPasscode }]
            : []),
          ...quest.steps.flatMap((s, i) =>
            s.passcode
              ? [{ id: s.id, label: `Round ${i + 1}`, code: s.passcode }]
              : [],
          ),
        ];
        return (
          <div className="dev-quest" key={quest.slug}>
            <div className="dev-row">
              <span>
                <strong>{quest.title}</strong> <code>{quest.slug}</code>{" "}
                <em className={`status-pill is-${state?.status}`}>
                  {state?.status ?? "…"}
                </em>
                {quest.status === "coming-soon" && (
                  <small> (coming-soon flag)</small>
                )}
              </span>
              {state && Object.keys(state.missing).length > 0 && (
                <small>missing {JSON.stringify(state.missing)}</small>
              )}
              <small>
                {saved
                  ? `step ${saved.step + 1}/${quest.steps.length} · ${Object.keys(saved.answers).length} answers · ${saved.completions} completion${saved.completions === 1 ? "" : "s"}`
                  : "no progress"}
              </small>
            </div>
            <div className="dev-row dev-wrap">
              <Btn
                mode="engine"
                label={`Start ${quest.slug}`}
                busy={r.busy}
                onClick={() =>
                  r.engine(`Start ${quest.slug}`, {
                    type: "quest.start",
                    slug: quest.slug,
                  })
                }
              ...
              <Btn
                mode="engine"
                label={`Complete ${quest.slug} (rewards)`}
                busy={r.busy}
                onClick={() =>
                  r.engine(`Complete ${quest.slug} (rewards)`, {
                    type: "quest.complete",
                    slug: quest.slug,
              ...
              <Btn
                mode="force"
                label={`Mark ${quest.slug} complete`}
                busy={r.busy}
                title="quest.completed with no rewards"
                onClick={() =>
                  r.force(`Mark ${quest.slug} complete`, {
              ...
              {view.save.world.unlockedQuests.includes(quest.slug) ? (
                <Btn
                  mode="force"
                  label={`Relock ${quest.slug}`}
                  busy={r.busy}
                  onClick={() =>
                    r.force(`Relock ${quest.slug}`, {
                      action: "relock-quest",
                      slug: quest.slug,
                    })
                  }
                ...
                <Btn
                  mode="force"
                  label={`Unlock ${quest.slug}`}
                  busy={r.busy}
                  onClick={() =>
                    r.force(`Unlock ${quest.slug}`, {
                      action: "unlock-quest",
                      slug: quest.slug,
                    })
                  }
              ...
              <Btn
                mode="force"
                label={`Reset ${quest.slug}`}
                busy={r.busy}
                danger
                onClick={() =>
                  r.force(`Reset ${quest.slug}`, {
                    action: "reset-quest",
                    slug: quest.slug,
                  })
                }
            ...
            {quest.steps.length > 0 && (
              <div className="dev-row dev-wrap">
                <label>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setJump((c) => ({
                        ...c,
                        [quest.slug]: Number(e.target.value),
                      }))
                    }
                  >
                    {quest.steps.map((s, i) => (
                ...
                <Btn
                  mode="force"
                  label={`Jump ${quest.slug}`}
                  busy={r.busy}
                  onClick={() =>
                    r.force(`Jump ${quest.slug}`, {
                      action: "jump-quest",
                      slug: quest.slug,
                      step: target,
                    })
                ...
                {quest.steps.map((s) => {
                  const done =
                    Boolean(saved?.answers[s.id]) ||
                    Boolean(saved?.unlockedSteps.includes(s.id));
                  return (
                    <Btn
                      key={s.id}
                      mode="force"
                      label={`Reset step ${s.id}`}
                      busy={r.busy}
                      onClick={() =>
                        r.force(`Reset step ${s.id}`, {
                          action: "reset-step",
                          slug: quest.slug,
                          stepId: s.id,
                        })
            ...
            {passcodes.length > 0 && (
              <div className="dev-row dev-wrap dev-passcodes">
                {passcodes.map((p) => (
                  <span key={p.id}>
                    <small>{p.label}</small>{" "}
                    <code className="passcode-value">{p.code}</code>
                  </span>
                ))}
              </div>
            )}
...
function WorldSection({ view, r }: { view: SaveView; r: Runner }) {
  ...
  const flagList = (
    ...
    <div className="dev-row dev-wrap">
      <span>
        {kind === "location" ? "Discovered locations" : "Secrets"} ({ids.length}
        )
      </span>
      {ids.map((id) => (
        <Btn
  ...
  return (
    <Section
      id="world"
      title="World flags"
      blurb="Quest unlocks only matter for quests whose requirements include `unlock: true`. Locations and secrets are free-form ids until content defines them."
    >
      <div className="dev-grid">
        <div className="dev-row dev-wrap">
          ...
          {quests.map((q) => {
            ...
            return (
              <Btn
                ...
                busy={r.busy}
                onClick={() =>
                  r.force(`${on ? "Relock" : "Unlock"} ${q.slug}`, {
                    action: on ? "relock-quest" : "unlock-quest",
                    slug: q.slug,
                  })
                }
              />
        ...
        {flagList(
          "location",
          view.save.world.discoveredLocations,
          location,
          setLocation,
        )}
...
function ScenariosSection({ r }: { r: Runner }) {
  ...
  return (
    <Section
      id="scenarios"
      title="Scenarios"
      blurb="Known states, each built from a wiped save: bootstrap, then the recipe's events. Every load replaces the current save."
    >
      <div className="dev-grid">
        <div className="dev-row">
          <label>
        ...
        {scenarios.map((s) => (
          <div className="dev-row" key={s.id}>
            <span>
              <strong>{s.name}</strong> <small>{s.description}</small>
            </span>
            <Btn
              mode="force"
              label={`Load ${s.name}`}
...
function SaveStateSection({ view }: { view: SaveView }) {
  ...
  useEffect(() => {
    refresh();
  }, [refresh, view.save.seq]);
  const raw = useMemo(() => JSON.stringify(view.save, null, 2), [view.save]);
  return (
    <Section
      id="save"
      title="Save state"
      blurb="The raw save file (a fold over the event log) and the most recent events, newest first."
    >
      <details className="dev-details" open>
        <summary>
          Raw save · seq {view.save.seq} · updated{" "}
          {new Date(view.save.updatedAt).toLocaleString()}
        </summary>
          ...
          <tbody>
            {(events ?? []).map((event) => {
              ...
              return (
                <tr key={event.seq}>
                  <td>{event.seq}</td>
                  <td>{event.at.slice(11, 19)}</td>
                  <td>
                    <code>{type}</code>
                  </td>
                  <td>
                    <small>{JSON.stringify(rest)}</small>
                  </td>
                  <td>
      ...
      <div className="dev-grid">
        <div className="dev-row dev-wrap">
          {resettableSystems.map((system) => (
            <Btn
              ...
              danger
              onClick={() =>
                r.force(`Reset ${system}`, { action: "reset-system", system })
              }
            />
          ))}
        </div>
        <div className="dev-row">
          ...
          <Btn
            ...
            onClick={() => {
              setConfirm("");
              void r.force("Wipe save", { action: "wipe" });
            }}
          />
        </div>
      </div>
  ...
  return (
    <main className="dev-page">
      <header className="dev-header">
        ...
        <p className="dev-legend">
          <span className="dev-badge is-engine">ENGINE</span> rule-checked
          command, exactly what gameplay does &nbsp;{" "}
          <span className="dev-badge is-force">FORCE</span> raw event appended
          to the log, bypasses rules
        </p>
        ...
        {r.log.length > 0 && (
          <ul className="dev-log" aria-live="polite">
            {r.log.map((entry) => (
              <li key={entry.at} className={entry.ok ? "is-ok" : "is-error"}>
                {entry.ok ? "✓" : "✗"} {entry.text}
              </li>
            ))}
          </ul>
