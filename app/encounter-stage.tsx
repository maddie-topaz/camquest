...
export function EncounterStage({ encounterId, onResult }: { encounterId: string; onResult: (result: EncounterResult) => void }) {
  const stage = useRef<HTMLDivElement>(null)
  const actor = useActorRef(encounterMachine, {
    input: { encounterId, getContainer: () => stage.current, load: loadEncounterRuntime },
  })
  const state = useSelector(actor, (snapshot) => (snapshot.matches('loading') ? 'loading' : snapshot.matches('playing') ? 'playing' : snapshot.matches('failed') ? 'failed' : 'finished'))
  ...
  return (
    <div className={`encounter-stage is-${state}`} data-encounter={encounterId}>
      <div ref={stage} className="encounter-canvas" aria-label="Encounter" role="application" />
      {state === 'loading' && <div className="encounter-overlay" role="status"><div className="loading-spinner"><span /><span /><span /><span /></div><p>Tuning in…</p></div>}
      {state === 'failed' && (
        <div className="encounter-overlay is-error">
          ...
          <button className="portal-button" onClick={() => actor.send({ type: 'RETRY' })}>Retry</button>
        </div>
      )}
    </div>
  )
