...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const error = useSelector(actor, (snapshot) => (snapshot.matches('failed') ? snapshot.context.error : null))
  const loading = useSelector(actor, (snapshot) => snapshot.matches('loading'))
  const syncing = useSelector(actor, (snapshot) => snapshot.matches('syncing') || snapshot.context.queue.length > 0)

  // Turns a queued job back into a promise by waiting for the machine to
  // emit its result under the id we queued it with.
  const enqueue = useCallback(
    (event: { type: 'COMMAND'; command: Command } | { type: 'ADMIN'; body: Record<string, unknown> }) =>
      new Promise<SaveView>((resolve, reject) => {
        const id = nextJobId()
        const done = actor.on('job.done', (emitted) => {
          if (emitted.id !== id) return
          done.unsubscribe(); failed.unsubscribe()
  ...
  const dispatch = useCallback((command: Command) => enqueue({ type: 'COMMAND', command }), [enqueue])
  const admin = useCallback((body: Record<string, unknown>) => enqueue({ type: 'ADMIN', body }), [enqueue])
      ...
      new Promise<SaveView>((resolve, reject) => {
        ...
        const subscription = actor.subscribe((next) => {
          if (next.context.view) {
            ...
            subscription.unsubscribe()
            reject(next.context.error instanceof Error ? next.context.error : new Error('Failed to load save'))
          }
        })
      }),
    [actor],
