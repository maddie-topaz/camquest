...
import { adventures, getAdventure, getProgress, resetProgress, saveProgress, siteConfig, type Adventure, type AdventureProgress, type ChallengeStep } from '@/lib/adventures'
...
function QuestLog() { const progress = useStoredProgress(); return <Shell><main className="relative z-10 mx-auto max-w-6xl px-5 pb-16"><div className="page-title"><p className="eyebrow">Cam&apos;s arcade</p><h1>Quest log</h1><p>Pick a chapter. Every button starts a little story.</p></div><div className="quest-grid">{adventures.map((adventure) => <QuestCard key={adventure.id} adventure={adventure} progress={progress[adventure.slug]} />)}</div></main></Shell> }
