...
/**
 * Plain password auth against a connection string (the local container).
 * @param {string} connectionString
 * @returns {import("pg").PoolConfig}
 */
export const localPoolConfig = (connectionString) => ({
  connectionString,
...
/**
 * The RDS database, authenticated with a short-lived IAM token.
 * @returns {import("pg").PoolConfig}
 */
...
export const databasePoolConfig = () =>
  process.env.DATABASE_URL ? localPoolConfig(process.env.DATABASE_URL) : rdsPoolConfig();
