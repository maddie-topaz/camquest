...
describe("quest.complete", () => {
  ...
  it("unlocks quests and earns condition achievements through rewards", () => {
    ...
    expect(
      handleCommand(next, { type: "quest.start", slug: "cams-gambit" }),
  ...
  it("rejects completing a locked or already-complete quest", () => {
    expect(
      ...
      rejection: {
        code: "locked",
        missing: { questsCompleted: ["unknown-signal"] },
      },
    });
    expect(
      handleCommand(
        ...
        { type: "quest.complete", slug: "cams-gambit", answers: {} },
      ),
    ).toMatchObject({ ok: false, rejection: { code: "already-completed" } });
  });
});
...
describe("quest.start / quest.progress", () => {
  ...
  it("starting twice is a no-op, progress is bounds-checked", () => {
    const started = saveFrom([
      created(),
      completed("unknown-signal"),
      { type: "quest.started", slug: "cams-gambit" },
    ]);
    expect(
      handleCommand(started, { type: "quest.start", slug: "cams-gambit" }),
    ).toEqual({ ok: true, events: [] });
    expect(
      handleCommand(started, {
        type: "quest.progress",
        slug: "cams-gambit",
        step: 99,
        answers: {},
    ...
    ).toMatchObject({ ok: false, rejection: { code: "bad-step" } });
  });
});
...
describe("grants.accept", () => {
  it("accepts only keys that are actually pending", () => {
    const save = saveFrom([
      created(),
      granted("cowbell", 1, "starter_loadout", "starter:cowbell"),
    ]);
    const result = handleCommand(save, {
      type: "grants.accept",
      grantKeys: ["starter:cowbell", "made-up"],
    });
    expect(result).toMatchObject({ ok: true });
    if (!result.ok) return;
    expect(result.events[0].payload).toEqual({
      type: "grants.accepted",
      grantKeys: ["starter:cowbell"],
    });
    expect(
      handleCommand(save, { type: "grants.accept", grantKeys: ["made-up"] }),
    ).toEqual({ ok: true, events: [] });
  });
});
