...
describe('dev events', () => {
  it('relock / forget undo their flags', () => {
    ...
    expect(save.world).toEqual({ unlockedQuests: [], discoveredLocations: [], secrets: [] })
  })

  it('system.reset clears exactly one slice of the save', () => {
    const base = [created('Cam'), granted('cowbell', 2), completed('unknown-signal'), { type: 'xp.gained' as const, amount: 300, reason: 't' }, { type: 'trait.changed' as const, trait: 'chaos', delta: 3, reason: 't' }, { type: 'achievement.unlocked' as const, achievementId: 'first-quest', reason: 't' }]
    for (const system of resettableSystems) {
      const save = saveFrom([...base, { type: 'system.reset', system, reason: 't' }])
      expect(save.player.name).toBe('Cam')
      if (system === 'inventory') { expect(save.player.inventory).toEqual({}); expect(save.player.grants).toEqual([]) } else expect(save.player.inventory.cowbell.quantity).toBe(2)
      if (system === 'xp') expect(save.player.xp).toBe(0); else expect(save.player.xp).toBe(300)
      if (system === 'traits') expect(save.player.traits.chaos).toBe(5); else expect(save.player.traits.chaos).toBe(8)
      if (system === 'quests') expect(save.quests).toEqual({}); else expect(save.quests['unknown-signal'].completions).toBe(1)
      if (system === 'achievements') expect(save.player.achievements).toEqual({}); else expect(save.player.achievements['first-quest']).toBeDefined()
    }
  })
})

describe('scenarios', () => {
  const play = (id: string, params = {}) => saveFrom(scenariosById[id].events(params))

  it('every scenario produces a save', () => {
    for (const scenario of scenarios) expect(() => play(scenario.id, { slug: 'cams-gambit', step: 2 })).not.toThrow()
  })
  ...
  it('quest-in-progress jumps to the step with earlier passcodes unlocked', () => {
    ...
    expect(progress.unlockedSteps).toEqual([START_UNLOCK_ID, 'load-cartridge', 'load-map', 'fate-engine'])
  ...
  it('everything-unlocked completes every quest and maxes traits', () => {
    ...
    expect(Object.values(save.quests).every((q) => q.completions > 0)).toBe(true)
  ...
  it('jumpToStep clamps and keeps existing answers', () => {
    ...
    expect(events[1].payload).toMatchObject({ type: 'quest.progressed', step: quest.steps.length - 1, answers: { 'load-cartridge': 'Sun Cartridge' } })
  })
})
