...
describe('reducer: encounter.completed', () => {
  it('keeps plays, best and last per encounter', () => {
    const save = saveFrom([
      created(),
      { type: 'encounter.completed', encounterId: 'signal-lock', questSlug: 'unknown-signal', stepId: 'lock-signal', score: 210 },
      { type: 'encounter.completed', encounterId: 'signal-lock', questSlug: 'unknown-signal', stepId: 'lock-signal', score: 120 },
    ])
    expect(save.player.encounters['signal-lock']).toMatchObject({ plays: 2, bestScore: 210, lastScore: 120 })
  })
...
describe('command: encounter.complete', () => {
  it('records the result and pays score-based rewards once', () => {
    ...
    const command = { type: 'encounter.complete' as const, questSlug: 'unknown-signal', stepId: 'lock-signal', encounterId: 'signal-lock', score: 320, operationId: 'op-1' }
    const result = handleCommand(save, command)
    if (!result.ok) throw new Error(result.rejection.message)
    const types = result.events.map((e) => e.payload.type)
    expect(types).toEqual(expect.arrayContaining(['encounter.completed', 'xp.gained', 'item.granted']))
    ...
    const again = handleCommand(next, { ...command, score: 480, operationId: 'op-2' })
    ...
    expect(commit(next, again.events).player.inventory['golden-key'].quantity).toBe(1)
  })
...
describe('questMachine: encounter step', () => {
  it('hides the button until the game reports, then turns the step over with the score', () => {
    ...
    const actor = createActor(questMachine, { input: { quest, save: readySave(), completed: false } }).start()
  ...
  it('restores a played encounter as already revealed', () => {
    const quest = getQuest('unknown-signal')!
    const actor = createActor(questMachine, { input: { quest, save: readySave(), completed: false, saved: { step: 0, answers: { 'lock-signal': '200' }, unlockedSteps: [], completions: 0 } } }).start()
...
describe('encounterMachine', () => {
  ...
  it('loads the runtime, mounts it, emits the result once, and unmounts', async () => {
    ...
    const runtime: EncounterRuntime = { mount: (_el, onComplete) => { report = onComplete; return unmount } }
    const load = vi.fn(async () => runtime)
    const results: number[] = []
    const actor = createActor(encounterMachine, { input: { encounterId: 'signal-lock', getContainer: () => container, load } })
  ...
  it('fails without a container or when the module is missing, and can retry', async () => {
    const noContainer = createActor(encounterMachine, { input: { encounterId: 'signal-lock', getContainer: () => null, load: async () => ({ mount: () => () => {} }) } }).start()
    await settle()
    expect(noContainer.getSnapshot().matches('failed')).toBe(true)

    const load = vi.fn().mockRejectedValueOnce(new Error('no scene')).mockResolvedValueOnce({ mount: () => () => {} })
    const actor = createActor(encounterMachine, { input: { encounterId: 'nope', getContainer: () => container, load } }).start()
    await settle()
  ...
  it('tears the game down if the machine stops mid-play', async () => {
    ...
    const actor = createActor(encounterMachine.provide({ actors: { loadRuntime: fromPromise(async (): Promise<EncounterRuntime> => runtime) } }), { input: { encounterId: 'signal-lock', getContainer: () => container, load: async () => runtime } }).start()
    await settle()
    actor.stop()
    expect(unmount).toHaveBeenCalledTimes(1)
  })
