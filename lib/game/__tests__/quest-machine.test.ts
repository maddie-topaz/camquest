...
const start = (
  overrides: Partial<Parameters<typeof questMachine.provide>[0]> = {},
  input?: Partial<{
    saved: NonNullable<ReturnType<typeof saveFrom>["quests"][string]>;
    completed: boolean;
    slug: string;
  }>,
) => {
  ...
  const actor = createActor(questMachine.provide(overrides), {
    input: {
      quest: getQuest(input?.slug ?? "cams-gambit")!,
      save,
      saved: input?.saved,
...
describe("questMachine", () => {
  ...
  it("unlocks with the right code (any case), persists, and opens the step", () => {
    ...
    expect(emitted.at(-1)).toMatchObject({
      type: "progress",
      progress: { step: 0, unlockedSteps: ["load-cartridge"] },
    });
  });
  ...
  it("mystery: select → lock choice → continue, committing the answer", () => {
    ...
    expect(questSelectors.selectedCard(snap())?.outcome).toContain(
      "Planet Royale",
    );
    ...
    const { actor } = start(
      {},
      {
        saved: {
          ...
          unlockedSteps: ["load-cartridge"],
          completions: 0,
        },
      },
    );
    expect(actor.getSnapshot().matches({ step: { open: "revealed" } })).toBe(
      true,
    );
  ...
  it("finishing the last step emits complete with every answer and ends", () => {
    const saved = {
      step: gambit.steps.length - 1,
      answers: { "load-cartridge": "Sun Cartridge" },
      unlockedSteps: gambit.steps.map((s) => s.id),
      completions: 0,
    };
    ...
    expect(emitted.at(-1)).toMatchObject({
      type: "complete",
      answers: {
        "load-cartridge": "Sun Cartridge",
        "final-stage": "Glass Garden",
      },
    });
  ...
  it("is locked when the save does not meet the quest requirements", () => {
    const actor = createActor(questMachine, {
      input: { quest: gambit, save: saveFrom([created()]), completed: false },
    }).start();
    expect(actor.getSnapshot().matches("locked")).toBe(true);
    expect(actor.getSnapshot().context.missing).toMatchObject({
  ...
  it("opens once the requirements are met", () => {
    const actor = createActor(questMachine, {
      input: {
        quest: gambit,
        save: saveFrom([created(), completed("unknown-signal")]),
        completed: false,
      },
    }).start();
