...
import { canConsume, missingRequirements, newlyEarnedAchievements, questStatus } from '../rules'
...
describe('inventory rules', () => {
  it('rejects unknown, non-consumable, and insufficient items', () => {
    const save = saveFrom([created(), granted('kitanas-blessing'), granted('cowbell')])
    expect(canConsume(save, 'nope', 1)).toMatchObject({ ok: false, code: 'unknown-item' })
    expect(canConsume(save, 'kitanas-blessing', 1)).toMatchObject({ ok: false, code: 'not-consumable' })
    expect(canConsume(save, 'cowbell', 2)).toMatchObject({ ok: false, code: 'insufficient' })
...
describe('prerequisites', () => {
  it('reports exactly what is missing, in requirement shape', () => {
    ...
    const missing = missingRequirements(save, { items: ['vip-wristband'], questsCompleted: ['cams-gambit'], traits: { chaos: 8 }, level: 3, unlock: true }, 'unknown-signal')
    expect(missing).toEqual({ items: ['vip-wristband'], questsCompleted: ['cams-gambit'], traits: { chaos: 8 }, level: 3, unlock: true })
  })
  ...
  it('is empty once everything is satisfied', () => {
    ...
    expect(missingRequirements(save, { items: ['vip-wristband'], questsCompleted: ['cams-gambit'], traits: { chaos: 8 }, level: 3, unlock: true }, 'unknown-signal')).toEqual({})
  })
})
...
describe('achievements', () => {
  it('earns condition achievements from the save alone', () => {
    ...
    expect(newlyEarnedAchievements(saveFrom([created(), completed('cams-gambit')]))).toContain('first-quest')
    expect(newlyEarnedAchievements(saveFrom([created(), granted('kitanas-blessing')]))).toContain('blessed-by-kitana')
  })

  it('does not re-earn one already recorded', () => {
    const save = saveFrom([created(), completed('cams-gambit'), { type: 'achievement.unlocked', achievementId: 'first-quest', reason: 't' }])
