...
class SignalLockScene extends Phaser.Scene {
  ...
  create() {
    ...
    g.strokeRoundedRect(TRACK_LEFT - 20, TRACK_Y - 34, TRACK_RIGHT - TRACK_LEFT + 40, 68, 8)
    g.lineStyle(1, 0x3b2a69, 1)
    for (let x = TRACK_LEFT; x <= TRACK_RIGHT; x += 25) g.lineBetween(x, TRACK_Y + 22, x, TRACK_Y + 30)

    this.windowZone = this.add.rectangle(this.windowCentre, TRACK_Y, this.windowHalf * 2, 56, CYAN, 0.28).setStrokeStyle(2, CYAN, 0.9)
    this.needle = this.add.rectangle(this.needleX, TRACK_Y, 6, 76, PINK)
    this.tweens.add({ targets: this.windowZone, alpha: { from: 0.7, to: 1 }, duration: 600, yoyo: true, repeat: -1 })

    this.add.text(WIDTH / 2, 48, 'SIGNAL LOCK', { ...FONT, fontSize: '30px', fontStyle: 'bold', color: '#ff5fbd' }).setOrigin(0.5)
    this.add.text(WIDTH / 2, 84, 'TAP WHEN THE NEEDLE CROSSES THE GLOW', { ...FONT, fontSize: '12px', color: '#c0b7d0' }).setOrigin(0.5)
    this.roundText = this.add.text(TRACK_LEFT - 20, 130, '', { ...FONT, fontSize: '14px', color: '#55e7ff' })
    this.scoreText = this.add.text(TRACK_RIGHT + 20, 130, '', { ...FONT, fontSize: '14px', color: '#ffd166' }).setOrigin(1, 0)
    this.feedback = this.add.text(WIDTH / 2, 300, '', { ...FONT, fontSize: '22px', fontStyle: 'bold' }).setOrigin(0.5)
  ...
  update(_time: number, delta: number) {
    ...
    if (this.needleX >= TRACK_RIGHT) { this.needleX = TRACK_RIGHT; this.direction = -1 }
    if (this.needleX <= TRACK_LEFT) { this.needleX = TRACK_LEFT; this.direction = 1 }
  ...
  private startRound() {
    ...
    this.windowCentre = Phaser.Math.Between(TRACK_LEFT + this.windowHalf + 30, TRACK_RIGHT - this.windowHalf - 30)
    this.windowZone.setPosition(this.windowCentre, TRACK_Y).setSize(this.windowHalf * 2, 56)
  ...
  private finish() {
    ...
    this.tweens.add({ targets: text, scale: { from: 1, to: 1.06 }, duration: 500, yoyo: true, repeat: -1 })
