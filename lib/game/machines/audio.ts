...
export const audioMachine = setup({
  ...
  actions: {
    ...
    applyVolume: ({ context }) => context.player.setVolume(context.muted ? 0 : context.volume),
    toggleMute: assign({ muted: ({ context }) => !context.muted }),
    setVolume: assign({ volume: ({ event }) => (event.type === 'SET_VOLUME' ? Math.min(1, Math.max(0, event.volume)) : 0.8) }),
  },
}).createMachine({
  id: 'audio',
  context: ({ input }) => ({ player: input.player, volume: input.volume ?? 0.8, muted: input.muted ?? false }),
  initial: 'locked',
  entry: 'applyVolume',
  on: {
  ...
  states: {
    ...
    ready: {
      on: {
        CUE: { actions: 'play' },
        TOGGLE_MUTE: { target: 'muted', actions: ['toggleMute', 'applyVolume'] },
      },
    },
    muted: {
      on: {
        TOGGLE_MUTE: { target: 'ready', actions: ['toggleMute', 'applyVolume'] },
      },
    },
  },
})
