...
export const playerMachine = setup({
  ...
  actors: {
    load: fromPromise(
      ({ input }: { input: { load: () => Promise<SaveView> } }) => input.load(),
    ),
    runJob: fromPromise(
      async ({
        input,
        ...
        input: {
          job: Job;
          run: PlayerMachineInput["run"];
          admin: PlayerMachineInput["admin"];
        };
      }) =>
        input.job.kind === "command"
          ? input.run(input.job.command)
          : input.admin(input.job.body),
    ),
  },
  guards: {
    ...
    enqueue: assign({
      queue: ({ context, event }) => {
        if (event.type === "COMMAND")
          return [
            ...context.queue,
            { id: event.id, kind: "command" as const, command: event.command },
          ];
        if (event.type === "ADMIN")
          return [
            ...context.queue,
            { id: event.id, kind: "admin" as const, body: event.body },
          ];
  ...
  states: {
    loading: {
      invoke: {
        ...
        onDone: {
          target: "ready",
          actions: assign({ view: ({ event }) => event.output, error: null }),
        },
        onError: {
          target: "failed",
      ...
      invoke: {
        ...
        onDone: {
          ...
          actions: [
            ...
            emit(({ context, event }) => ({
              type: "job.done" as const,
              id: context.queue[0].id,
              view: event.output,
            })),
            "dequeue",
          ],
