...
export type QuestProgress = { step: number; answers: Record<string, string>; unlockedSteps: string[] }
...
const normalise = (value: string | undefined) => (value ?? '').trim().toUpperCase()

const progressOf = (context: QuestContext): QuestProgress => ({ step: context.stepIndex, answers: context.answers, unlockedSteps: context.unlockedSteps })
...
export const questMachine = setup({
  ...
  actions: {
    unlockStep: assign(({ context }) => {
      const step = currentStep(context)!
      return {
        unlockedSteps: context.unlockedSteps.includes(step.id) ? context.unlockedSteps : [...context.unlockedSteps, step.id],
        passcodeInput: '',
        passcodeError: false,
      }
    }),
    rejectPasscode: assign({ passcodeError: true }),
    typePasscode: assign({ passcodeInput: ({ event }) => (event.type === 'TYPE_PASSCODE' ? event.value : ''), passcodeError: false }),
    typeAnswer: assign({ answer: ({ event }) => (event.type === 'TYPE_ANSWER' ? event.value : '') }),
    ...
    advance: assign(({ context }) => {
      const step = currentStep(context)!
      const answers = context.answer ? { ...context.answers, [step.id]: context.answer } : context.answers
      const stepIndex = context.stepIndex + 1
      return { answers, stepIndex, answer: answers[context.quest.steps[stepIndex]?.id] ?? '', passcodeInput: '', passcodeError: false }
    }),
  ...
  context: ({ input }) => {
    ...
    const stepIndex = Math.min(restored?.step ?? 0, Math.max(0, input.quest.steps.length - 1))
  ...
  states: {
    deciding: {
      always: [
        { guard: 'questLocked', target: 'locked' },
        { target: 'step' },
      ],
    },
    locked: { type: 'final' },
...
export const questSelectors = {
  ...
  isRevealed: (snapshot: Snapshot) => snapshot.matches({ step: { open: 'revealed' } }),
  isLastStep: (snapshot: Snapshot) => snapshot.context.stepIndex >= snapshot.context.quest.steps.length - 1,
