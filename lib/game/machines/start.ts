...
export type RevealItem = PendingGrantView & { grantKeys: string[]; reasons: string[] }

export const groupPendingGrants = (pending: PendingGrantView[]): RevealItem[] => {
...
export type StartEvent = { type: 'START' } | { type: 'ACCEPT' } | { type: 'RETRY' }
...
export const startMachine = setup({
  ...
  actors: {
    boot: fromPromise(async ({ input }: { input: { load: () => Promise<SaveView>; minBootMs: number } }) => {
      const [view] = await Promise.all([input.load(), new Promise((resolve) => setTimeout(resolve, input.minBootMs))])
      return view
    }),
    accept: fromPromise(async ({ input }: { input: { accept: (keys: string[]) => Promise<unknown>; grantKeys: string[] } }) => {
  ...
  delays: {
    ...
    revealReady: ({ context }) => REVEAL_READY_BASE_MS + context.items.length * REVEAL_ITEM_DELAY_MS,
  },
  actions: {
    cue: emit((_, params: { cue: string; detail?: Record<string, unknown> }) => ({ type: 'cue' as const, cue: params.cue, detail: params.detail })),
    enter: emit({ type: 'enter' as const }),
  },
}).createMachine({
  id: 'start',
  context: ({ input }) => ({ items: [], minBootMs: input.minBootMs ?? 850, load: input.load, accept: input.accept }),
  initial: 'idle',
  states: {
    idle: {
      on: { START: 'booting' },
    ...
    booting: {
      ...
      invoke: {
        src: 'boot',
        input: ({ context }) => ({ load: context.load, minBootMs: context.minBootMs }),
        onDone: {
          actions: assign({ items: ({ event }) => groupPendingGrants(event.output.pendingGrants) }),
          target: 'deciding',
        },
        onError: {
          target: 'error',
    ...
    accepting: {
      invoke: {
        src: 'accept',
        input: ({ context }) => ({ accept: context.accept, grantKeys: context.items.flatMap((item) => item.grantKeys) }),
        onDone: {
          target: 'accepted',
...
export const startSelectors = {
  ...
  isFirstGrant: (items: RevealItem[]) => items.length > 0 && items.every((item) => item.reasons.every((reason) => reason === 'starter_loadout')),
}
