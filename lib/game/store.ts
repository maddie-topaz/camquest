...
const loadWithClient = async (
  ...
  let save: SaveFile = snapshot.rows[0]?.state
    ? normaliseSave(snapshot.rows[0].state)
    : emptySave(playerId);
...
export const wipeSave = (playerId: PlayerId = DEFAULT_PLAYER, name = "Cam") =>
  inTransaction(async (client) => {
    await client.query(`DELETE FROM game_saves WHERE player_id = $1`, [
      playerId,
    ]);
    await client.query(`DELETE FROM game_events WHERE player_id = $1`, [
      playerId,
    ]);
